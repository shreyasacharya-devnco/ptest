...
class Media {
  ...
  x!: number;
  speed: number = 0;
  hoverGray: { current: number; target: number } = { current: 1, target: 1 };
    ...
    this.program = new Program(this.gl, {
      ...
      fragment: `
        ...
        void main() {
          ...
          vec3 finalColor = mix(color.rgb, vec3(luma), uGrayscale);
  ...
  update(scroll: { current: number; last: number }, direction: 'right' | 'left') {
    ...
    this.hoverGray.current = lerp(this.hoverGray.current, this.hoverGray.target, 0.1);
    this.program.uniforms.uGrayscale.value = this.hoverGray.current;
  ...
  onResize({ screen, viewport }: { screen?: ScreenSize; viewport?: Viewport } = {}) {
    ...
    // Square planes so the rounded-box mask (with uBorderRadius near 0.5)
    // forms a clean circle with no corner clipping showing the canvas behind.
    const planeSize = (this.viewport.height * (880 * this.scale)) / this.screen.height;
    this.plane.scale.y = planeSize;
    this.plane.scale.x = planeSize;
...
class App {
  ...
  start: number = 0;
  pointer: { x: number | null; y: number | null } = { x: null, y: null };
  ...
  // Approximates the pointer's world-space X by inverting the same linear
  // screen->viewport mapping used to size/position the planes — close enough
  // for hover detection without a full perspective unprojection.
  updateHover() {
    if (!this.medias) return;
    let worldX: number | null = null;
    if (this.pointer.x !== null) {
      worldX = (this.pointer.x / this.screen.width - 0.5) * this.viewport.width;
    }
    this.medias.forEach(media => {
      const isHovered = worldX !== null && Math.abs(media.plane.position.x - worldX) < media.plane.scale.x / 2;
      media.hoverGray.target = isHovered ? 0 : 1;
    });
  }
...
export interface CircularGalleryHandle {
  // Drives the gallery's scroll position directly from an external scroll
  // progress value (0 -> 1 covers exactly one full loop through the unique items).
...
const CircularGallery = forwardRef<CircularGalleryHandle, CircularGalleryProps>(function CircularGallery({
  ...
  useImperativeHandle(ref, () => ({
    setProgress(progress: number) {
      ...
      const loopLength = app.medias[0].width * (app.mediasImages.length / 2);
      app.scroll.target = baseScrollRef.current + progress * loopLength;
    }
  }), []);
  ...
  useEffect(() => {
    ...
    resolveFont(font, fontUrl).then(resolvedFont => {
      ...
      appRef.current = app;
      baseScrollRef.current = app.scroll.target;
    });
    return () => {
      isMounted = false;
      appRef.current = null;
  ...
  }, [items, bend, textColor, borderRadius, font, fontUrl, scrollSpeed, scrollEase, disableWheel]);
